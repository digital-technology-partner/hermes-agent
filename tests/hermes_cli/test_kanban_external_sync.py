...
def test_external_terminal_sync_fails_closed_when_claim_lands_before_transition(
    kanban_home,
    monkeypatch,
    external_status,
    helper_name,
):
    task_id = f"claim-race-{external_status}"
    with kb.connect() as conn:
        kb.sync_external_task(
            conn,
            task_id=task_id,
        ...
        original_helper = getattr(kb, helper_name)
        claimed_run_id = None
        ...
        monkeypatch.setattr(kb, helper_name, claim_then_transition)
    ...
    assert task is not None
    assert task.status == "running"
    assert task.current_run_id == claimed_run_id
    owned_run = next(run for run in runs if run.id == claimed_run_id)
    assert owned_run.ended_at is None
    assert owned_run.status == "running"
